use crate::{ContextEvent, CapturePolicy};
...
impl RollingContext {
    ...
    fn prune(&mut self, now: &str) {
        ...
        self.events.retain(|event| parse_epoch_seconds(&event.timestamp) >= cutoff);
...
fn parse_epoch_seconds(value: &str) -> u64 {
    ...
    if value.len() >= 20 && value.as_bytes().get(4) == Some(&b'-') {
        let year = value.get(0..4).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
        let month = value.get(5..7).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
        let day = value.get(8..10).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
        let hour = value.get(11..13).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
        let minute = value.get(14..16).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
        let second = value.get(17..19).and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
