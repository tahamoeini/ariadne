...
pub fn import_legacy_investigation_json(
    ...
    let id = string(source, "id")
        .ok_or_else(|| StorageError::InvalidLegacy("Investigation id is missing".into()))?;
    ...
    let mut thread = Thread::new(name, created_at)
        .map_err(|error| StorageError::InvalidLegacy(error.to_string()))?;
    thread.id = id;
    ...
    thread.checkpoint = source.get("checkpoint").and_then(|value| {
        Some(ariadne_core::Checkpoint {
            text: string(value, "text")?,
            created_at: string(value, "createdAt").unwrap_or_else(|| default_created_at.clone()),
        })
    });
    ...
    if let Some(files) = snapshot.get("editedFiles").and_then(Value::as_array) {
        for file in files.iter().filter_map(Value::as_str) {
            add_artifact(&mut thread, file, 0, 1, &default_created_at);
        }
    }
    ...
    if let Some(references) = source.get("browserReferences").and_then(Value::as_array) {
        ...
        thread.references = references
            ...
            .filter_map(|reference| {
                policy.sanitize_reference(ExternalReference {
                    url: string(reference, "url")?,
                    title: string(reference, "title"),
                    captured_at: string(reference, "capturedAt")
    ...
    if let Some(timeline) = source.get("timeline").and_then(Value::as_array) {
        for entry in timeline {
            if let Some(mapped) = import_timeline_entry(entry, &thread) {
                thread.timeline.push(mapped.0);
                if let Some(event) = mapped.1 {
...
fn add_artifact(thread: &mut Thread, reference: &str, visits: u32, edits: u32, timestamp: &str) {
        ...
        .find(|artifact| artifact.id == id)
    {
        ...
        thread.artifacts.push(ContextArtifact {
            ...
            safe_reference: reference.into(),
            first_observed: timestamp.into(),
            last_observed: timestamp.into(),
            visit_count: visits,
            edit_count: edits,
        });
    }
    ...
    let nodes = value
        ...
        .map(|nodes| {
            nodes
                .iter()
                .filter_map(|node| {
                    let file = string(node, "filePath")?;
                    Some(GraphNode {
                        artifact_id: format!("file:{file}"),
                        last_observed_at: string(node, "lastObservedAt")
    ...
    let edges = value
        ...
        .map(|edges| {
            edges
                ...
                .filter_map(|edge| {
                    Some(GraphEdge {
                        from_artifact_id: format!("file:{}", string(edge, "fromFilePath")?),
                        to_artifact_id: format!("file:{}", string(edge, "toFilePath")?),
                        relationship: match string(edge, "relationship").as_deref() {
                            Some("definition") => GraphRelationship::Navigation,
                            Some("reference") => GraphRelationship::ExplicitReference,
                            _ => GraphRelationship::Transition,
                        },
                        count: edge.get("count").and_then(Value::as_u64).unwrap_or(1) as u32,
                        last_observed_at: string(edge, "lastObservedAt")
                            .unwrap_or_else(|| timestamp.into()),
                    })
                })
                .take(1_000)
...
fn import_timeline_entry(
    value: &Value,
    thread: &Thread,
) -> Option<(TimelineEntry, Option<ContextEvent>)> {
    ...
    let artifact_id = file_path.as_ref().map(|path| format!("file:{path}"));
    let event = ContextEvent {
        id: format!("legacy:{timestamp}:{kind}"),
        timestamp: timestamp.clone(),
        event_type: event_type.clone(),
        application: None,
        artifact: file_path.map(|path| ariadne_core::ArtifactRef {
            kind: ArtifactKind::File,
        ...
        workspace: thread
            .workspace
            .clone()
            .map(|path| ariadne_core::ArtifactRef {
